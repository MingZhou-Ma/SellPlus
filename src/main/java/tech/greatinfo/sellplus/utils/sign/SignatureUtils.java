 ...
 * </html>
 */
package tech.greatinfo.sellplus.utils.sign;
...
import tech.greatinfo.sellplus.common.constants.SystemConstant;
...
* @Author： - Jason   
* @CreatTime：2018年8月20日 下午6:34:47   
* @Modify By：   
...
public class SignatureUtils {
	...
	public static String buildSortJson(Map<String, String> dataMap) {
		...
		for (String jsonKey : jsonKeys) {
			if (sortSb.length() > 0) {
				sortSb.append(SystemConstant.STRING_AND_CHAR);
			}
			sortSb.append(jsonKey).append(SystemConstant.STRING_ASSIGN_CHAR).append(dataMap.get(jsonKey));
		}
	...
	/**
	 * @Description: Test  -- 用于后期的sign加签验签操作   需要生成一对保存 同时更新凭证操作
	 * @param args 
	 * @Autor: Jason
	 */
	public static void main(String[] args) {
		try {
			...
			System.out.println(rsa256CheckContent("jasonandy@hotmail.com", sign, getPublicKey(map)));/**/
		} catch (Exception e) {
			e.printStackTrace();
		}
